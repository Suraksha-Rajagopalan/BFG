...
% Compute connectivity matrix (cosine similarity)
...
for i = 1:size(epochs, 3) % Loop over channels
    for j = 1:size(epochs, 3) % Loop over channels
        if i ~= j % Exclude self-connectivity
            % Compute cosine similarity between channels i and j
            dot_product = sum(epochs(:,:,i) .* epochs(:,:,j), 1); % Compute dot product
            norm_i = sqrt(sum(epochs(:,:,i) .^ 2, 1)); % Compute norm of channel i
            norm_j = sqrt(sum(epochs(:,:,j) .^ 2, 1)); % Compute norm of channel j
            similarity_measure = dot_product ./ (norm_i .* norm_j); % Compute cosine similarity
            connectivity_measure = mean(similarity_measure); % Take the mean over all samples
